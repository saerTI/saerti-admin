...
export default function IncomeDataList() {
  ...
  const handleBulkImport = async (data: any[]): Promise<{ success: boolean; message: string; data?: any }> => {
    try {
      // Agregar el income_type_id a cada registro
      const incomesWithType = data.map(row => {
      ...
      const result = await incomeDataService.bulkCreate(incomesWithType);
  ...
  const getTemplateData = () => {
    ...
    return {
      columns,
      sheetName: currentType?.name || 'ingresos'
    };
  };
