// src/pages/DynamicExpense/CategoryTable.tsx
...
const CategoryTable = forwardRef<CategoryTableHandle, CategoryTableProps>(({ expenseTypeId }, ref) => {
  ...
  const loadCategories = async () => {
    ...
    try {
      setLoading(true);
      const data = await expenseCategoryService.getByType(expenseTypeId);
      setCategories(data);
    } catch (error: any) {
  ...
  const handleCategoryChange = (index: number, field: keyof ExpenseCategory, value: any) => {
  ...
  useImperativeHandle(ref, () => ({
    saveAll: async (typeId?: number) => {
      ...
      try {
        ...
        const savedCategories = await expenseCategoryService.getByType(targetTypeId);
        ...
        for (const saved of savedCategories) {
          ...
          if (!stillExists && saved.id) {
            await expenseCategoryService.delete(saved.id);
          }
        }
        ...
        for (const category of categories) {
          ...
          if (category.id) {
            ...
            // Create new
            const result = await expenseCategoryService.create(targetTypeId, category);
      ...
      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        ...
        <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
          {categories.map((category, index) => (
            <tr key={index} className={!category.id ? 'bg-red-50 dark:bg-red-900/10' : ''}>
