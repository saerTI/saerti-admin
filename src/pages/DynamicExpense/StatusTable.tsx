// src/pages/DynamicExpense/StatusTable.tsx
import { useState, useEffect, useImperativeHandle, forwardRef } from 'react';
import { TrashBinIcon, PlusIcon } from '@/icons';
import { expenseStatusService, type ExpenseStatus } from '@/services/expenseStatusService';
...
const StatusTable = forwardRef<StatusTableHandle, StatusTableProps>(({ expenseTypeId }, ref) => {
  ...
  const loadStatuses = async () => {
    ...
    try {
      setLoading(true);
      const data = await expenseStatusService.getByType(expenseTypeId);
      setStatuses(data);
    } catch (error: any) {
  ...
  const handleStatusChange = (index: number, field: keyof ExpenseStatus, value: any) => {
  ...
  useImperativeHandle(ref, () => ({
    saveAll: async (typeId?: number) => {
      const targetTypeId = typeId || expenseTypeId;
      if (!targetTypeId) {
        throw new Error('Debe guardar el tipo de egreso primero');
      }

      setLoading(true);
      try {
        // Get currently saved statuses from backend
        const savedStatuses = await expenseStatusService.getByType(targetTypeId);
        ...
        for (const saved of savedStatuses) {
          ...
          if (!stillExists && saved.id) {
            await expenseStatusService.delete(saved.id);
          }
        }
        ...
        for (const status of statuses) {
          ...
          if (status.id) {
            // Update existing
            await expenseStatusService.update(status.id, status);
          } else {
            // Create new
            const result = await expenseStatusService.create(targetTypeId, status);
            status.id = result.id;
          }
        }
      ...
      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        ...
        <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
          {statuses.map((status, index) => (
            <tr key={index} className={!status.id ? 'bg-red-50 dark:bg-red-900/10' : ''}>
              <td className="px-3 py-2">
                <input
                  type="text"
                  value={status.name}
                  onChange={(e) => handleStatusChange(index, 'name', e.target.value)}
                  className="w-full px-2 py-1 border border-gray-300 dark:border-gray-600 rounded focus:ring-2 focus:ring-red-500 dark:bg-gray-700 dark:text-white text-sm"
                  placeholder="Nombre"
                  autoComplete="off"
                />
              </td>
          ...
          <tr className="bg-gray-50 dark:bg-gray-800/50">
            <td colSpan={5} className="px-3 py-2">
              <button
                ...
                className="flex items-center space-x-1 px-3 py-1.5 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors text-sm w-full justify-center"
                disabled={loading}
              >
