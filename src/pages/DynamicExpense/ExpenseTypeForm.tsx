// src/pages/DynamicExpense/ExpenseTypeForm.tsx
...
interface FieldConfig {
  ...
  label: string;
  showKey: keyof ExpenseType;
  requiredKey: keyof ExpenseType;
}
...
export default function ExpenseTypeForm() {
  ...
  const { refreshExpenseTypes } = useSidebar();
  ...
  const loadExpenseType = async () => {
    try {
      ...
      setError(err.message || 'Error cargando tipo de egreso');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // First save or create the expense type
      let expenseTypeId: number;
      if (isEditing) {
        console.log('🔵 Updating expense type:', id);
        await expenseTypeService.update(Number(id), formData);
        expenseTypeId = Number(id);
        console.log('✅ Expense type updated');
      } else {
        console.log('🔵 Creating new expense type');
        const result = await expenseTypeService.create(formData);
        expenseTypeId = result.id;
        console.log('✅ Expense type created with ID:', expenseTypeId);
      }

      // Then save statuses and categories (both for create and edit)
      if (expenseTypeId) {
        console.log('🔵 Saving statuses for type:', expenseTypeId);
        console.log('🔵 Status ref current:', statusRef);

        if (statusRef) {
          await statusRef.saveAll(expenseTypeId);
        ...
        if (formData.show_category) {
          console.log('🔵 Saving categories for type:', expenseTypeId);
          console.log('🔵 Category ref current:', categoryRef);

          if (categoryRef) {
            await categoryRef.saveAll(expenseTypeId);
      ...
      // Refresh sidebar and navigate to expense types list
      refreshExpenseTypes();
      setTimeout(() => navigate('/egresos/tipos'), 1000);
    } catch (err: any) {
      console.error('❌ Error saving:', err);
      setError(err.message || 'Error guardando tipo de egreso');
    } finally {
      setLoading(false);
    }
  };

  const handleFieldToggle = (showKey: keyof ExpenseType, requiredKey: keyof ExpenseType) => {
  ...
  const handleRequiredToggle = (requiredKey: keyof ExpenseType, showKey: keyof ExpenseType) => {
    ...
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
            {isEditing ? 'Editar Tipo de Egreso' : 'Crear Tipo de Egreso'}
          </h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Configure los campos que desea utilizar para este tipo de egreso
          </p>
        </div>
        ...
        <div className="flex space-x-3">
          <button
            type="button"
            onClick={() => navigate('/egresos/tipos')}
      ...
      <div className="space-y-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="Ej: Compras, Servicios, Arriendos..."
                required
                autoComplete="off"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-red-500 dark:bg-gray-700 dark:text-white"
                placeholder="Descripción opcional del tipo de egreso"
                autoComplete="off"
              />
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
            Define los diferentes estados que puede tener un egreso de este tipo
          </p>
          <StatusTable ref={statusTableRef} expenseTypeId={id ? Number(id) : undefined} />
        </div>
          ...
          {formData.show_category && (
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Define las categorías disponibles para este tipo de egreso
            </p>
              <CategoryTable ref={categoryTableRef} expenseTypeId={id ? Number(id) : undefined} />
            </div>
          )}
        </div>
