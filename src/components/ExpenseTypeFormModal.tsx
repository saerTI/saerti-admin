// src/components/ExpenseTypeFormModal.tsx
import { useState, useEffect, useRef } from 'react';
import { expenseTypeService } from '@/services/expenseTypeService';
import type { ExpenseType } from '@/types/expense';
import { useSidebar } from '@/context/SidebarContext';
import StatusTable, { type StatusTableHandle } from '@/pages/DynamicExpense/StatusTable';
import CategoryTable, { type CategoryTableHandle } from '@/pages/DynamicExpense/CategoryTable';
...
export default function ExpenseTypeFormModal({ isOpen, onClose, expenseTypeId, onSuccess }: ExpenseTypeFormModalProps) {
  const { refreshExpenseTypes } = useSidebar();
  const isEditing = Boolean(expenseTypeId);
  ...
  useEffect(() => {
    if (isOpen && isEditing && expenseTypeId) {
      loadExpenseType();
  ...
  }, [isOpen, isEditing, expenseTypeId]);

  const loadExpenseType = async () => {
    try {
      setLoading(true);
      const data = await expenseTypeService.getById(expenseTypeId!);
      setFormData(data);
    } catch (err: any) {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      let typeId: number;
      if (isEditing) {
        await expenseTypeService.update(expenseTypeId!, formData);
          ...
          <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-red-500 dark:bg-gray-700 dark:text-white"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                ...
                <StatusTable ref={statusTableRef} expenseTypeId={expenseTypeId} />
              </div>
              ...
              {formData.show_category && (
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                  ...
                  <CategoryTable ref={categoryTableRef} expenseTypeId={expenseTypeId} />
                </div>
              )}
            </div>
          </div>
