// saer-frontend/src/components/CC/ImportPrevisionalesModal.tsx

import React, { useState, useEffect, useCallback } from 'react';
import { previsionalesService } from '../../services/CC/previsionalesService';
import { ImportResponse } from '../../types/CC/previsional';
import { handlePrevisionalExcelUpload } from '../../utils/previsionalUtils';
import { generatePrevisionalTemplate } from '../../utils/previsionalesExcelTemplateGenerator';
...
export const ImportPrevisionalesModal: React.FC<Props> = ({ isOpen, onClose, onImportSuccess }) => {
  ...
  const handleImport = async () => {
    ...
    try {
      const previsionalesToImport = await handlePrevisionalExcelUpload(selectedFile);
      
      if (previsionalesToImport.length === 0) {
        throw new Error("El archivo está vacío o no se encontraron datos válidos.");
      }

      const response = await previsionalesService.importPrevisionales(previsionalesToImport);
        ...
        <div className="sticky top-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-semibold text-gray-800 dark:text-white">
              Importación Masiva de Pagos Previsionales
            </h2>
            <button
              onClick={onClose}
                    ...
                    {importResult.results && (
                      <div className="mt-2">
                        <ul className="list-disc pl-5 text-sm text-gray-700 dark:text-gray-300 space-y-1">
                          ...
                          <li className="text-green-700 dark:text-green-300">Registros exitosos: {importResult.results.success}</li>
